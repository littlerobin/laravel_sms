				...
				<div class="logo_part text-left col-12 col-md-3 pl-0 d-flex flex-column justify-content-center">
					<!-- <div class="select_service">
						<div id="notificationsDropdownButton" class="service_selector pt-3 pb-3 pl-2 pr-2 pl-sm-3 pr-sm-3 pl-lg-4 pr-lg-4 d-flex flex-row align-items-center justify-content-between pointer">
							...
							<img class="hidden-md-up" src="{{asset('laravel_assets/images/front/img/img3.svg')}}" width="60" alt="">
					...
					<div class="logo_holder pl-2 pl-lg-4 mt-3 mb-3 hidden-sm-down">
						<img src="{{asset('laravel_assets/images/front/img/img3.svg')}}" width="110" alt="">
						<h3 class="mt-sm-1 mb-sm-2 mt-lg-3 mb-lg-4">click to call</h3>
						<p>{{trans('main.crud.let_website_connect')}}</p>
					</div>
				</div>
				<div class="slick_videos_holder col-12 col-md-6 p-0">
					<slick slides-to-show=1 class="slick-for" settings="slickVideosConfig" ng-if="CTCVideosObj.tutor.length && tutor">
						<div ng-repeat="video in CTCVideosObj.tutor track by $index">
							<iframe width="100%" height="335" ng-src="@{{getVideoFromPlaylist(video)}}" id="vm-video-@{{$index}}" frameborder="0" allowfullscreen></iframe>
						</div>
					</slick>
					<slick slides-to-show=1 class="slick-for" settings="slickVideosConfig" ng-if="CTCVideosObj.promo.length && promo">
						<div ng-repeat="video in CTCVideosObj.promo track by $index">
							<iframe width="100%" height="335" ng-src="@{{getVideoFromPlaylist(video)}}" id="vm-video-@{{$index}}" frameborder="0" allowfullscreen></iframe>
						</div>
					</slick>
				</div>
					...
					<div class="tabs">
						...
						<div class="tab_content hidden-sm-down">
							<div class="tutor" ng-if="tutor">
								<slick infinite=true slides-to-show=@{{CTCVideosObj.tutor.length}} settings="slickVideosNav" ng-if="CTCVideosObj.tutor" class="slick-nav">
							...
							<div class="promo" ng-if="promo">
								<slick infinite=true slides-to-show=@{{CTCVideosObj.promo.length}} settings="slickVideosNav" ng-if="VMVideosObj.promo" class="slick-nav">
									<div class="item pl-2 pr-2 d-flex flex-row justify-content-center align-items-center pointer" ng-repeat="item in CTCVideosObj.promo">
										<p>@{{item.title.substring(item.title.indexOf("-") + 1).slice(0, -6)}}</p>
										<div class="line"></div>
										<div class="triangle"></div>
									</div>
